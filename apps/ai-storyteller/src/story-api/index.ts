...
app.get("/", (c) => {
  return c.json({
    ...
    recent_success: {
      ...
      choice_interactive:
        '✅ "Wizard\'s Tower or Enchanted Forest?" choice working',
    },
...
app.post(
  "/start-story",
  zValidator("json", StartStoryRequestSchema),
  async (c) => {
    try {
      ...
      safeLog(
        env,
        "info",
        "New story started with SmartInference and ElevenLabs",
        {
          story_id: storyId,
          child_name: "Emma",
          age: request.age,
...
app.post(
  "/continue-story",
  zValidator("json", SubmitChoiceRequestSchema),
  async (c) => {
    try {
      ...
      const currentSegment = await dbService.getStorySegment(
        request.segment_id
      );
        ...
        try {
          ...
          if (storyMatch) {
            storyData = {
              story_text: storyMatch[1].trim(),
              choice_question:
                choiceMatch && choiceMatch[1].trim() !== "null"
                  ? choiceMatch[1].trim()
                  : null,
            };
          } else {
      ...
      let storyContext = await memoryService.getStoryContext(
      ...
      if (!storyContext) {
        ...
        storyContext = await memoryService.getStoryContext(
          currentSegment.story_id.toString()
        );
      }
      ...
      try {
        audioUrl = await ttsService.generateSpeech(storyData.story_text);
        const timestamp = Date.now();
        audioKey = `story-${currentSegment.story_id}-segment-${
          currentSegment.segment_order + 1
        }-${timestamp}.mp3`;
        ...
        console.log(
          "Audio generation failed (likely quota exceeded), continuing without audio:",
          audioError
        );
      ...
      const updatedContext = await memoryService.getStoryContext(
        currentSegment.story_id.toString()
      );
      if (
        updatedContext &&
        updatedContext.segments &&
        updatedContext.segments.length > currentSegment.segment_order &&
        updatedContext.segments[currentSegment.segment_order]
      ) {
        updatedContext.segments[currentSegment.segment_order]!.choice_made =
          request.audio_blob;
        await memoryService.storeStoryContext(
          currentSegment.story_id.toString(),
          updatedContext
        );
      }
      ...
      const errorMessage =
        error instanceof Error ? error.message : "Unknown error";
      safeLog(c.env, "error", "Failed to continue story", {
        error: errorMessage,
      });
      return c.json(
        {
  ...
  try {
    ...
    await memoryService.addStorySegment(
      testStoryId,
      {
        ...
        choice_question:
          "Should TestChild touch the glowing flowers or talk to the fairy lights?",
    ...
    return c.json({
      ...
      memory_features_working: {
        enhanced_context: !!context,
        narrative_tracking:
          (context?.narrative_arc?.characters_introduced?.length || 0) > 0,
...
app.post("/test-continue-story", async (c) => {
  try {
    ...
    const testStory = {
      story_text: `Emma decided to ${
        choice || "explore"
      } and discovered something amazing! The path led to a hidden clearing where magical creatures were dancing in the moonlight. One of them, a friendly fairy, offered Emma a special gift.`,
      choice_question:
        "Should Emma accept the fairy's gift or ask about the magical creatures?",
    };
...
app.get("/test-tts", async (c) => {
  try {
    ...
    const testText =
      "Hello Emma, this is a test of the ElevenLabs text to speech service.";
    const apiKey = env.ELEVENLABS_API_KEY;
    ...
    if (
      ...
      if (
        ...
        return c.json({
          status: "✅ AI Working!",
          ai_response: content,
          model: "deepseek-r1-distill-qwen-32b (via Vultr)",
          timestamp: new Date().toISOString(),
          note: "Successfully using Vultr Inference API with native fetch",
          response_format: "OpenAI-compatible chat format",
        });
      }
    }
...
app.get("/get-audio/:audioKey", async (c) => {
  try {
    ...
    const response = await fetch(
      `https://api.elevenlabs.io/v1/text-to-speech/${audioInfo.voice_id}`,
      {
        method: "POST",
        headers: {
          Accept: "audio/mpeg",
    ...
    if (url.pathname === "/test-env-direct") {
      ...
      return new Response(
        JSON.stringify({
          ...
          has_ai: !!env.AI,
          story_db_type: (env as any).STORY_DB_V2
            ? typeof (env as any).STORY_DB_V2
            : "undefined",
          ai_type: env.AI ? typeof env.AI : "undefined",
          // Test if STORY_DB has executeQuery
          story_db_has_executequery: (env as any).STORY_DB_V2
            ? typeof ((env as any).STORY_DB_V2 as any).executeQuery
            : "method check",
        }),
        {
